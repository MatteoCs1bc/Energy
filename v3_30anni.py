...
def _serie_pesata(df, pesi_colonne, scala=1.0, clip_upper=1.0):
    ...
    if clip_upper is not None: serie = serie.clip(upper=clip_upper)
...
def _mappa_profilo_annuale_su_indice(profilo_orario, indice_target):
    ...
    for ts in indice_target:
        ...
        if chiave in mappa: valori.append(mappa[chiave])
        elif ts.month == 2 and ts.day == 29: valori.append(mappa.get((2, 28, ts.hour), mappa.get((3, 1, ts.hour), 0.0)))
        else: valori.append(0.0)
...
def leggi_gme(file_gme):
    ...
    col = df_gme.columns[2]
    if df_gme[col].dtype == 'object': df_gme[col] = df_gme[col].astype(str).str.replace('.', '', regex=False).str.replace(',', '.', regex=False)
    df_gme[col] = pd.to_numeric(df_gme[col], errors='coerce')
    df_gme['Ora'] = pd.to_numeric(df_gme['Ora'], errors='coerce')
    data_convertita = pd.to_datetime(df_gme['Data'], dayfirst=True, errors='coerce')
    df_gme['Datetime'] = data_convertita + pd.to_timedelta(df_gme['Ora'] - 1, unit='h')
    df_gme = df_gme.dropna(subset=['Datetime', col]).set_index('Datetime')
    df_gme.rename(columns={col: 'Fabbisogno_MW'}, inplace=True)
    return df_gme[['Fabbisogno_MW']]

@st.cache_data
def carica_profili_rinnovabili(file_fotovoltaico, file_eolico):
    df_pv = pd.read_csv(file_fotovoltaico, parse_dates=['time']).set_index('time')
    df_wind = pd.read_csv(file_eolico, parse_dates=['time']).set_index('time')
    return {
        'pv_nord': pd.Series(_serie_pesata(df_pv, PV_WEIGHTS_NORD, 1000.0).values, index=df_pv.index),
        'pv_sud': pd.Series(_serie_pesata(df_pv, PV_WEIGHTS_SUD, 1000.0).values, index=df_pv.index),
        'wind_nord': pd.Series(_serie_pesata(df_wind, WIND_WEIGHTS_NORD, 1.0).values, index=df_wind.index),
        'wind_sud': pd.Series(_serie_pesata(df_wind, WIND_WEIGHTS_SUD, 1.0).values, index=df_wind.index),
    }
...
def calcola_capacita_anno_rate(anno, start_yr, val_start, val_target, rate, step_wise=False):
    ...
    if val_target < val_start: # Smantellamento / Fine vita
        valore = val_start - (anni_attivi * rate)
        if valore < val_target: valore = val_target
    else: # Costruzione
        valore = val_start + (anni_attivi * rate)
        if valore > val_target: valore = val_target
...
def simula_motore_30_anni(array_pv, array_wind, array_fabbisogno, t_start_py, rate_py, anni_transizione=30):
    ...
    scenari_nuc_gw = get_valid_targets(nuc_sq, max_nuc, [0, 5, 10, 15, 20])
...
def applica_economia_cumulata(risultati_30y, fabbisogno_annuo_mwh, mercato, anni_transizione=30):
    ...
    df_risultati['Sogni_Infranti'] = (
        ...
        abs(df_risultati['Target_BESS'] - df_risultati['Reached_BESS']) +
        abs(df_risultati['Target_Nuc'] - df_risultati['Reached_Nuc'])
    )
...
# ==========================================
st.title("⚡ SIMULATORE TRANSIZIONE ENERGETICA")
...
try:
    ...
    with st.spinner("Calcolo degli scenari in corso..."):
    ...
    st.subheader(f"🏆 Scenario Ottimizzato (all'anno {anni_transizione})")
    col1, col2, col3 = st.columns(3)
    col1.metric("Costo Medio Bolletta", f"{miglior_config['Costo_Medio_30y']:.1f} €/MWh")
    col2.metric("Carbon Intensity Media", f"{miglior_config['Carbon_Intensity_30y']:.1f} gCO₂/kWh")
    col3.metric("Spesa Totale Cumulata", f"{miglior_config['Spesa_Totale_Mld_30y']:.1f} Mld €")
    
    st.markdown(
        f"**Capacità finale:** {miglior_config['Reached_PV']} GW Solare | "
        f"{miglior_config['Reached_Wind']} GW Eolico | {miglior_config['Reached_BESS']} GWh BESS | "
    ...
    fig = px.scatter(
        ...
        hover_data=['Reached_PV', 'Reached_Wind', 'Reached_BESS'],
        labels={
            'Carbon_Intensity_30y': f"Emissioni Medie (gCO₂/kWh)",
            'Spesa_Totale_Mld_30y': f"Spesa Totale Sistema (Miliardi €)",
            'Reached_Nuc': "Nuc (GW)"
        }
    )
    fig.add_trace(go.Scatter(
        x=[miglior_config['Carbon_Intensity_30y']], y=[miglior_config['Spesa_Totale_Mld_30y']],
        mode='markers', marker=dict(color='lime', size=15, line=dict(color='black', width=2)), name='Ottimo Scelto'
    ))
